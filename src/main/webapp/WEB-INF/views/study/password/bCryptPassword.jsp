...
<jsp:include page="/WEB-INF/views/include/slide2.jsp" />
	...
	<div class="container">
		...
		<pre>
			- 스프링 시큐리티(Spring Security) 프레임워크에서 제공하는 클래스 중 암호화 하는데 사용
			  주로 자바 서버프로그램 개발을 위해 필요한 '인증 / 권한부여' 및 '보안기능'을 제공해주는 프레임워크에 속한다.
			- BCryptPasswordEncoder는 BCrypt해싱함수를 사용하여 비밀번호를 인코딩해주는 메서드와
			  사용자에 의해 제출된 비밀번호를 저장소에 저장된 비밀번호와의 일치여부를 확인해주는 메서드로 제공된다.
			- BCryptPasswordEncoder는 PasswordEncoder 인터페이스를 구현한 클래스이다.
			- 사용되는 메서드 ?
			  1) encode(java.lang.CharSequence) : 패스워드를 암호화 해주는 메서드
			     반환타입 : String
			     encode() 메서드는 sha-1에 의한 8Byte(64Bit)로 결합된 해시키를 랜덤하게 생성된 솔트(salt)를 지원한다.
			  2) machers(java.lang.CharSequence)
			     제출된 인코딩 되지 않는 패스워드의 일치여부를 판단하기 위해 인코딩된 패스워드와 비교 판단한다.
			     반환타입 : boolean
		</pre>
		<hr />
		<p>
